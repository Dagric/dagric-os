...
- **Thirty-four wallpaper packs** in `/usr/share/wallpapers` — twenty designs,
  fourteen of them shipping a logo-free "Clean" twin. Each pack is a
  ...
  Three waves:
  - *Wave 1* — hue variations on one composition: Dagric, DagricDawn,
    DagricDusk, DagricForest, DagricMidnight, DagricNeon. These were once
    hand-made orphans with no generator; they have since been fitted
    numerically and folded back into `branding/wallpaper/make-wallpapers.sh`,
    so every pack in the tree is now reproducible from that one script. They
    ship branded-only (no Clean twin) purely to stay under the ~70 MB art
    ceiling — flipping their `CLEAN=0` costs about 9.7 MB.
  - *Wave 2* — varies the *composition*: DagricSlate (corner light, graphite),
  ...
  - *Wave 3* — varies the *structure*: DagricContour (iso-lines over a random
    terrain), DagricMesh (soft blob gradient), DagricPrism (flat low-poly
    facets), DagricLinen (woven cloth), DagricVoid (true black, for OLED),
    DagricHalftone (dot matrix).

  Waves 2 and 3 each ship a Clean twin.
- **Two Dagric colour schemes**: `DagricLight.colors`, `DagricDark.colors`.
- **Twelve user avatars** in `/usr/share/plasma/avatars` plus an SDDM
  `default.face.icon`, generated by `branding/avatars/make-avatars.sh`. There
  were none before, which meant every new account landed on a stock grey
  silhouette.
- Seven hand-drawn look wireframes exist as SVGs on the marketing site
  (`site/assets/look-*.svg`).

> **Note, later than the rest of this section.** The last line here used to
> read "nothing in the shipped image shows the user a picture of anything."
> That is no longer true: `dagric-appearance` ships the picture gallery this
> roadmap argues for, and `dagric-firstrun` puts wallpaper and layout
> thumbnails in front of the owner at first login. The analysis below is kept
> because the *reasoning* is what makes the next decision, but read section 1
> as history, not as a description of the current image.
