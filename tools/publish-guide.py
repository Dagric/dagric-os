...
def publish():
    ...
    # and it stays.
    #
    # THE WEB COPY MUST NOT. dagric.com is dark-only — site.css declares no light
    # band at all — so on a light-themed machine, which is the Windows default and
    # therefore the default for exactly the audience this site is written for,
    # clicking "Guide" in the nav flipped the page from near-black to near-white.
    # One nav click, two identities. It reads as a broken link rather than as a
    # feature.
    #
    # `not all and (…)` is the standard idiom for a block that must never match;
    # the original condition is left visible beside it so the next reader can see
    # what was disabled and why, rather than finding a deleted section.
    ...
    if os.path.exists(s):
        ...
        css, n_light = re.subn(
            ...
            '@media not all and (prefers-color-scheme: light)', css)
        if n_light:
            css = ("/* PUBLISHED COPY — generated by tools/publish-guide.py.\n"
                   "   Edit config/includes.chroot/usr/share/dagric/guide/guide.css,\n"
                   "   not this file.\n"
                   "   %d light-mode block(s) neutralised with `not all and`: the\n"
                   "   website is dark-only, so a guide that followed the visitor's\n"
                   "   system theme turned white one nav click from a near-black site.\n"
                   "   The offline copy inside the OS keeps them, deliberately. */\n"
                   % n_light) + css
        open(os.path.join(DST, "guide.css"), "w",
             encoding="utf-8", newline="\n").write(css)
        print("  asset   guide.css  (%d light-mode block(s) neutralised for the web)"
              % n_light)
